...
algorithm add_float(input uint$float_size$ f1, input uint$float_size$ f2, output uint$float_size$ res){
    uint$mantissa_size * 2+1$ max_mantissa(0);
    uint$mantissa_size * 2+1$ min_mantissa(0);

    uint$exponant_size$ max_exponant(0);
    
    uint$mantissa_size * 2+1$ sum_mantissa(0);
    ...
    if(f1[$float_size-1$,1]==f2[$float_size-1$,1]){  
        ...
        max_mantissa = (f1_inf_f2)?
                        {1b0,f2[0, $mantissa_size$], $mantissa_size$b0}: 
                        {1b0,f1[0, $mantissa_size$], $mantissa_size$b0};

        sum_mantissa = max_mantissa + min_mantissa;
        if(sum_mantissa[$mantissa_size * 2$,1]){
            res ={f1[$float_size-1$,1], max_exponant+1, sum_mantissa[$mantissa_size$,$mantissa_size$]};
        }else{
